...
##taking only second expression after "<=>"
b1<-lapply(a$V1,function(x) strsplit(x,"<=>",fixed=TRUE)[[1]][2])
##Taking only the indices of expressions containing the matching string "H2O["
c<-grep("H2O[",b,value=FALSE,fixed=TRUE)
c1<-grep("H2O[",b1,value=FALSE,fixed=TRUE)
c<-c(c,c1)
Result1<-length(c)
...
##Taking only 1the indices of expressions containing the matching string "H2O["
